### cook_procedures.get_placement(parameter)
          ...
          CASE
          ### CPA - Лидогенерация
            WHEN `cook_procedures.get_cpa`(parameter) IS NOT NULL 
            -- WHEN LOWER(calltracking) LIKE "%лидогенераторы%" 
            THEN `cook_procedures.get_cpa`(parameter) 
          ### Контекст
          ...
          ### Базы недвижимости 
            ...
            WHEN REGEXP_CONTAINS(LOWER(parameter), r'novostroy-m_базы') THEN 'Новострой-М'
