...
def segments(campaign_global):
    campaign = is_none(campaign_global)
    if re.search(r'epk-direct', campaign):
        return 'ЕПК (единая кампания)'
    elif re.search(r'autotargeting', campaign) and not re.search(r'compet|brend', campaign) and not re.search(r'pol.gon', campaign):
      ...
      return 'Комнатность'
    elif re.search(r'compet', campaign) and not re.search(r'ipoteka|rassrochka', campaign) and not re.search(r'pol.gon', campaign):
      return 'Конкуренты'
    elif re.search(r'mobapp', campaign):
      return 'Приложение'
    elif re.search(r'rassrochka', campaign):
      return 'Рассрочка'
    elif re.search(r'remarketing|retargeting|olv|alfabank|posetiteli', campaign) and not re.search(r'smartbanners', campaign):
      return 'Ретаргетинг'
      ...
      return 'DSA'
    elif re.search(r'generic', campaign) and not re.search(r'invest|intent|ipoteca|1com|2com|3com|compet|discovery|rassrochka|brend|tec|geo|rlsa', campaign) and not re.search(r'pol.gon', campaign):
      return 'Generic'
      ...
      return 'RLSA'
    elif re.search(r'rlsa', campaign):
      return 'RLSA'
    elif re.search(r'pol.gon', campaign):
        return 'Полигон'
    return 'Остальное'
