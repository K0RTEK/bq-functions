...
def get_ad_id(date, utm_source, utm_medium, utm_campaign, utm_content, utm_term, ct_campaign_name):
    if get_channel(date, utm_source, utm_medium, utm_campaign, utm_content, utm_term, ct_campaign_name) == "Контекстная реклама":
        return re.search(r'\\|b:([0-9]+)', utm_content).group(1) if re.search(r'\\|b:([0-9]+)', utm_content) else None
    return None
