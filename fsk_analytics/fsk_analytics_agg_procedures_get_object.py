...
def get_object(parameter):
    ...
    if re.search(r'realist', parameter):
        ...
        return 'ФСК Аутлет'
    elif re.search(r'brand|_soc_fsk_|soc_fsk-|fsk lider|бренд фск', parameter) and re.search(r'^mg|fsk.*недвижимость|fsk.*ретаргетинг', parameter):
        return 'Бренд ФСК'
    ...
    elif (re.search(r'(_| |-)dsk|dsk(_| |-)| дск|дск ', parameter) or parameter == 'dsk') and not re.search(r'olv|bitca', parameter):
        return 'Бренд ДСК'
    ...
    elif re.search(r'dvijen|dvizhen|движение', parameter) and not re.search(r'btl|говорово|govorovo', parameter):
        return 'Движение'
