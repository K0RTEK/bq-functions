...
def get_device(date, utm_source, utm_medium, utm_campaign, utm_content, utm_term, ct_campaign_name):
    if get_channel(date, utm_source, utm_medium, utm_campaign, utm_content, utm_term, ct_campaign_name) == "Контекстная реклама":
        return (re.search(r'dt:(.+?)\\|', utm_content) or re.search(r'dt:(.+?)$', utm_content)).group(1).upper() if re.search(r'dt:(.+?)\\|', utm_content) or re.search(r'dt:(.+?)$', utm_content) else None
    return None
