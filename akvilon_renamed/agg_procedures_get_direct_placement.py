...
def get_direct_placement(date, utm_source, utm_medium, utm_campaign, utm_content, utm_term, ct_campaign_name):
    if get_channel(date, utm_source, utm_medium, utm_campaign, utm_content, utm_term, ct_campaign_name) == "Контекстная реклама":
        placement = re.search(r'\\|s:(.+?)\\|', utm_content) or re.search(r'\\|s:(.+?)$', utm_content)
        return placement.group(1) if placement else None
    return None
