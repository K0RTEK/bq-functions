### raw_placements.get_objects(LOWER(parameter),res_type)
  CASE
		### Москва
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'жк0') AND REGEXP_CONTAINS(LOWER(parameter),r'msk|mmo')
			THEN CASE
				WHEN res_type = 'object' THEN 'ПРИГОРОД'
				WHEN res_type = 'crm_direction' THEN 'Москва'
				ELSE 'Москва'
			END
		WHEN REGEXP_CONTAINS(LOWER(parameter),r'cl-nmsk_|кластер - новая москва')
			THEN CASE
		...
		WHEN REGEXP_CONTAINS(LOWER(parameter),r'cl-vostok_|кластер - восток')
			THEN CASE
				...
				ELSE 'Москва'
			END
		WHEN REGEXP_CONTAINS(LOWER(parameter),r'cl-bus_|кластер - бизнес-класс')
			THEN CASE
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'nova|nova select') AND NOT REGEXP_CONTAINS(LOWER(parameter),r'novaya')
  	...
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'novograd.pavlino|новоград павлино')
			THEN CASE
				...
				ELSE 'Москва'
			END
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'oktyabrskaya.98|октябрьская.98')
			THEN CASE
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'tomilino|томилино')
			THEN CASE
				...
				ELSE 'Москва'
			END
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'troparevo|тропарево парк')
			THEN CASE
  	...
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'astrid|астрид')
			THEN CASE
				...
				ELSE 'Регионы'
			END	
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'spb brand|ipt.spb|mbl.sbp|spb.samolet|samolet.spb.*dinrem|спб общий')
			THEN CASE
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'lagolovo|лаголово')
  	...
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'nevskaya.dolina|невская долина')
			THEN CASE
				...
				ELSE 'Регионы'
			END
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'rybacko|живи(.|..)в рыбацком')
			THEN CASE
				WHEN res_type = 'object' THEN 'ЖИВИ В Рыбацком'
				WHEN res_type = 'crm_direction' THEN 'Спб'
				ELSE 'Регионы'
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'toriki|квартал торики')
			THEN CASE
				...
				ELSE 'Регионы'
			END
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'yuzhnaya.neva|южная нева')
			THEN CASE
				WHEN res_type = 'object' THEN 'Южная Нева'
				WHEN res_type = 'crm_direction' THEN 'Спб'
				ELSE 'Регионы'
			END
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'charkova|чаркова.*72')
			THEN CASE
				WHEN res_type = 'object' THEN 'Чаркова, 72'
				WHEN res_type = 'crm_direction' THEN 'Тюмень'
				ELSE 'Регионы'
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'celeport|clprt_samolet|clprt|целепорт')
  	...
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'dmitrov.dom|дмитров')
			THEN CASE
				...
				ELSE 'Сервисы'
			END
  	WHEN REGEXP_CONTAINS(LOWER(parameter),r'life.samolet|life') AND NOT REGEXP_CONTAINS(LOWER(parameter),r'samoletum')
			THEN CASE
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'senler')
			THEN CASE
				...
				ELSE 'Москва'
			END	
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'_flash|flash sale|flashsale')
			THEN CASE
				WHEN res_type = 'object' THEN 'Flash sale - общебренд'
				WHEN res_type = 'crm_direction' THEN 'Москва'
				ELSE 'Москва'
			END		
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'mbl_msk|_mbl_|квартиры с мебелью')
			THEN CASE
				WHEN res_type = 'object' THEN 'Квартиры с мебелью'
				WHEN res_type = 'crm_direction' THEN 'Москва'
				ELSE 'Москва'
			END		
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'ipt_msk|ipt-|ипотека')
			THEN CASE
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'lead-vk-engineer|lead-ovik-engineer|main-engineer|hr вакансии')
			THEN CASE
				...
				ELSE 'Сервисы'
			END		
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'samolet.*_ret|rem_samolet_ret|lead-p_minpay.vid_samolet_ct_ndv_lead_mmo_23_55_2|общебренд ретаргетинг|общий ремаркетинг')
			THEN CASE
				WHEN res_type = 'object' THEN 'Общебренд ретаргетинг'
				WHEN res_type = 'crm_direction' THEN 'Москва'
				ELSE 'Москва'
			END		
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'samolet.*d.nre|samolet.lal|mg_vka_soc_samolet_msk_traf|mg_samolet_msk_lead_lead-30d-1875-mmo_banner_02-24|mg_samolet_msk_lead_sub-1875-mmo_banner_02-24|mg_samolet_msk_lead_sub-2565-mmo_banner_02-24|общебренд динрем|samolet_msk_traf_dinrem')
			THEN CASE
				WHEN res_type = 'object' THEN 'Общебренд динрем'
				WHEN res_type = 'crm_direction' THEN 'Москва'
				ELSE 'Москва'
			END	
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'_installment|рассрочка')
			THEN CASE
    ...
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'petsflats')
			THEN CASE
				...
				ELSE 'Москва'
			END	
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'_detmetr|детский метр')
			THEN CASE
				WHEN res_type = 'object' THEN 'Детский метр'
				WHEN res_type = 'crm_direction' THEN 'Москва'
				ELSE 'Москва'
			END	
    WHEN REGEXP_CONTAINS(LOWER(parameter),r'brand.msk|samolet_msk|ipt_msk|mbl_msk|zhk_samolet_ct_vk|samoletgroup')
			THEN CASE
